...
SELECT last_name, count(last_name) AS actors FROM actor
GROUP BY last_name
HAVING COUNT(last_name) > 1;

-- 3. Using the rental table, find out how many rentals were processed by each employee.

SELECT staff_id, COUNT(rental_id) AS rentals_procesed FROM rental
GROUP BY(staff_id);

-- 4. Using the film table, find out how many films were released each year.

SELECT release_year, COUNT(title) AS films FROM film
...
SELECT rating, ROUND(AVG(length),2) AS avg_dur FROM film
GROUP BY rating;
